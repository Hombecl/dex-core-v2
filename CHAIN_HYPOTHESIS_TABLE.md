...
| DG5 | lp_account_initial_callback_minimum_supply_commit | initial_liquidity / state_persistence | Initial LP callback handling computes supply and reserves before minimum-supply and mint-output checks; a near-minimum or zero-liquidity boundary could commit reserves without a matching LP balance across the callback chain. | 5 | PRE_KILLED_DUPLICATE |

## DH fresh variant-parameter spiral clusters

| Cell | Production mechanism P | Failure class C | Initial hypothesis | Priority | Status |
|---|---|---|---|---:|---|
| DH1 | weighted_const_product_side_weight_complement | variant_parameter / asset_binding | Weighted constant-product admin parameters derive the opposite-side weight with a fixed-point complement when `side_wallet` selects token1; a side/address boundary could write the weight to the wrong asset or bypass the 0<w<ONE_DEC domain before the Pool save. | 1 | KILLED_TRUSTED_ON_TRUSTED |
| DH2 | weighted_stableswap_amp_rate_setter_tuple | variant_parameter / privileged_state | Weighted stableswap admin parameters carry amp, rate, weight, and setter through a nested ref; a zero-rate preserve rule or setter address form could commit a partially validated tuple and change future invariant authority. | 2 | KILLED_TRUSTED_ON_TRUSTED |
| DH3 | stableswap_internal_set_params_ref_tail_carry | parser_boundary / privileged_state | Stableswap Pool internal parameter handling loads response, amp, and carried excess without terminal parsing; a trusted Router body tail could shift the saved amp or residual TON recipient across the variant boundary. | 3 | KILLED_TRUSTED_ON_TRUSTED |
| DH4 | lp_account_accumulated_leg_callback_order | dual_leg / state_persistence | LPAccount accumulates token legs before deciding whether to call the Pool; an ordered single-sided pair could clear one stored leg before a callback/refund branch and expose a stale balance to a later callback. | 4 | pending |
| DH5 | router_vault_pay_to_zero_side_stateinit | asset_binding / state_init | Router pay_vault chooses a Vault from the first positive output side and sends the sum; a zero-side or StateInit tuple boundary could bind a referral deposit to the wrong token Vault while retaining the Pool sender. | 5 | pending |
