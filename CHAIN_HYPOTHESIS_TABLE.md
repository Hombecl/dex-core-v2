...
| BB2 | lp_wallet_bounce_truncated_body_parse | parser_boundary / bounce_accounting | A protocol-marked bounce with a truncated body could pass the bounce header path, parse an allowed opcode/amount prefix, and restore a malformed amount without a matching outbound debit. | 2 | pending |
| BB3 | router_pay_to_custom_payload_32bit_cutoff | parser_boundary / nested_route | Router pay_to treats payloads longer than 32 bits as cross-swap candidates; an exact 32/33-bit boundary could switch between ordinary transfer and nested route with different owner, amount, or excess semantics. | 3 | pending |
